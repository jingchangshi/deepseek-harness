...
由于该上游以文件路径而非图像内容返回截图，其提供方会向 [MCP 客户端](../../packages/mcp/mcp-client/README.zh.md)提供一个结果投影：在模型声明支持图像输入的路由上，该文件会被存为持久附件并以图像形式到达模型；在其他路由上则保留为带路径的文本诊断。同一提供方还把 Browser Harness 自身的使用指引发布到[技能注册表](skills.zh.md)，而不再需要手工导出。
