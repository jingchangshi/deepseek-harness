...
- **组合插件可以增强单个结果。** `projectResult` 是可选的、仅供程序化使用的钩子，它在标准投影之后追加内容。它存在的原因是某些服务器以引用的方式返回持久资源——例如截图文件路径——而任何协议规则都无法把它变成模型可见内容；了解该约定的提供方提供该钩子，本客户端则对任何特定服务器保持无知。该钩子绝不会收到失败的结果，且它自身的失败会降级为诊断信息，因此增强不可能把已完成的调用变成错误。
...
导出的 `createMcpToolDefinition(ctx, options)` 将上游工具 schema 和原始结果回调适配到相同的规范值、错误和持久化图像投影。每次回调都收到原样的 `ToolExecution`，包括其 Agent 和取消信号；SDK 的规范类型校验会在投影前检查返回结果。调用方负责注册、取消截止时间和提供方卸载。原生 Cua Driver 提供方使用此适配函数，无需打开 MCP 传输。传入 `projectResult` 会额外向每个成功结果追加调用方提供的内容；何时适用参见[理解实现](#understand-the-implementation)。
