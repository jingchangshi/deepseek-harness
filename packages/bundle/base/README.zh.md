...
已挂载的文件系统也会获得本地模式的[持久工作区身份](../../execution/execution-world/README.zh.md)，分配协调路径位于 `DSH_HOME/locks` 下。远程文件系统组合必须替换 `execution-world-identity` 行的完整配置，显式指定部署 UUID 和宿主协调路径。独立的 `sdk-minimal` profile 不挂载此服务。
