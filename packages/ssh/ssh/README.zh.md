---
description: "面向组合文件、进程与沙箱提供方的部署者，说明 OpenSSH 客户端配置及 POSIX 远端辅助进程生命周期。"
...
`dsh-ssh` 将 Windows 或 POSIX Harness 主机连接到 POSIX SSH 主机上已安装的辅助程序。部署方持有的 OpenSSH 主机别名提供认证与主机身份；配套的文件系统、子进程和沙箱提供方共享该连接。连接在就绪前验证已安装产物的摘要；辅助程序在连接关闭或租期到期时负责远端清理。
...
远端必须运行 Linux 或 macOS。POSIX 客户端需要连接复用与 Unix 套接字转发；Windows 客户端为每条流启动独立的 OpenSSH 进程，并使用固定的 Node 中继连接远端辅助程序的 Unix 套接字。启动前配置主机别名、凭据与已知主机记录：本服务启用 `BatchMode`、要求严格检查主机密钥、禁用认证代理转发，且不提供交互认证流程。
...
在 POSIX 客户端上，OpenSSH 主连接承载私有管理 RPC，并通过独立的 Unix 套接字与 SSH 通道转发每条程序流。在 Windows 客户端上，普通 SSH 进程承载管理 RPC，每条程序流由独立 SSH 进程调用固定的 Node 中继连接辅助程序的 Unix 套接字。程序 stdout 无法伪造管理回复，也不会占用控制流的通道窗口；POSIX 连接复用仍可能共享传输拥塞。
...
连接释放会先等待转发与取消子进程，以及尚在建立的流结束，再删除本地资源。Windows 的逐流 SSH 进程会被终止并等待退出；若终止未完成，将在有界时间后强制退出。传输丢失会拒绝待处理操作并使连接失效。辅助进程在 SSH EOF、终止信号或心跳到期时启动托管清理。断连客户端无法确认远端结果；操作不会自动重连或重放。

启动或进程结果失败后，预留项在原生进程范围清空后释放；有界完成缓存保留原始拒绝结果，供后续读取。辅助程序关闭也会等待已在进行的端点与目录清理。

辅助进程最多保留 64 个已发布的读取根目录。`fs.rootClose` 拥有独立管理容量，普通请求无法阻止作用域清理。发布前被取消的读取作用域会被关闭；关闭辅助进程时，先等待根目录操作并关闭持有的作用域，再释放文件系统提供方。即使其他释放操作成功，根目录关闭失败也会使辅助进程拒绝清理确认。调用方 API 与传输限制见[远端根目录读取](../fs-ssh/README.zh.md)。

`joinRemoteCleanupIfClosing()` 在连接可用时返回 false，仅在辅助进程确认成功关闭后确认清理完成。它在主动关闭期间等待，在传输丢失或确认失败导致清理结果未知时拒绝；后续本地释放不能撤销已经成功的确认。
...
- 不提供 Windows 远端端点、自动配置远端环境、重连或重放。
