...
`openReadRoot` 请求辅助进程持有的原生读取作用域。子项请求只携带其不透明 ID、逻辑路径分量和文本字节上限；宿主从不解析子项路径。辅助进程必须支持可选的[根目录读取器](../../fs/fs/README.zh.md)，否则打开失败，不回退到本地。`close()` 取消代理请求并仅关闭一次远端作用域。直接关闭失败时，仅已确认的辅助进程整体清理可以满足关闭要求；连接仍可用时保留原始错误，未获确认的传输丢失则以清理结果未知错误拒绝。传输丢失使作用域失效，不重放请求；辅助进程关闭时等待已发布及仍在打开的作用域。根目录文本传输限制为 8 MiB，不提供流式读取或 Git 约束。
