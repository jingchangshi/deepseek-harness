...
根作用域确认其不可变的 `aliasPolicy`。默认的 `follow-contained` 允许目标仍位于根目录内的别名；`deny` 在每次打开后代对象时拒绝符号链接和重解析点。要求 `deny` 的调用方必须验证确认值，并关闭不匹配的作用域。该策略不根据其他名称对硬链接内容进行分类。

`supportsRootRead(fs)` 检测可选的 `FsRootReadable` 能力；普通读取不意味着支持此能力。`openReadRoot` 返回调用方持有的 `FsReadRoot`，提供 `stat`、有界 `readText`、`listDir` 和幂等 `close`。请求使用逻辑路径分量而非提供方路径。提供方固定根目录对象，防止别名替换将读取重定向到目录之外；调用方必须关闭作用域。不支持的提供方不提供基于路径名的回退。参见[本地提供方支持](../fs-local/README.zh.md)。
